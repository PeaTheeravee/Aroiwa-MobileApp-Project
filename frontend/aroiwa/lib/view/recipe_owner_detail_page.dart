...
class RecipeOwnerDetailPage extends StatefulWidget {
  final int recipeId;

  const RecipeOwnerDetailPage({super.key, required this.recipeId});

  @override
  State<RecipeOwnerDetailPage> createState() => _RecipeOwnerDetailPageState();
}

class _RecipeOwnerDetailPageState extends State<RecipeOwnerDetailPage> {
  String recipeName = '';
  String recipeImg = '';
  ...
  Future<void> fetchRecipes() async {
    ...
    final url = Uri.parse('http://localhost:8000/recipes/$recipeId');
  ...
  Future<void> fetchRecipeUser() async {
    final url = Uri.parse('http://localhost:8000/users/$recipeUserId');
    final response = await http.get(url);
  ...
  Future<void> fetchPersonalScore() async {
    ...
    final url =
        Uri.parse('http://localhost:8000/scores/personal/$userId/$recipeId');
  ...
  Future<bool> postScoreCheck(int recipeId) async {
    ...
    final url =
        Uri.parse('http://localhost:8000/scores/canpost/$userId/$recipeId');
  ...
  Future<void> postScore(int score, int recipeId) async {
    final url = Uri.parse('http://localhost:8000/scores/create');
    final prefs = await SharedPreferences.getInstance();
    final userId = prefs.getInt('user_id');
    final accessToken = prefs.getString('access_token');
    ...
    if (accessToken != null) {
      if (userId != recipeUserId) {
        if (canPost) {
          ...
          final url = Uri.parse(
              'http://localhost:8000/scores/updatescore/$userId/$recipeId/$score');
  ...
  void deleteRecipe() async {
    ...
    final url = Uri.parse('http://localhost:8000/recipes/$recipeId/delete');
    ...
    try {
      ...
      print('Error occurred while delete: $e');
    }
  }
